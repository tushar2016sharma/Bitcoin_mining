...
```{r}
# Load data
df <- read_csv("Cleaned_bitcoin_mining_price.csv")
summary(df)
```
...
Estimated_MtCO2e_plot <- ggplot(df, aes(Date, Estimated_MtCO2e)) + 
  geom_line(col = 'blue') + 
  labs(title = 'Estimated_MtCO2e', x = 'Year') +
  scale_y_continuous(
    name = "Estimated_MtCO2e",
    sec.axis = sec_axis(trans = ~ . * 1, 
                        breaks = c(0, 50, 150),
                        labels = c('0', '50', '150'))
  )

combined_plot <- plot_grid(bitcoin_plot, Estimated_MtCO2e_plot, align = "v", ncol = 1)
